...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        max_val = float("-inf")
        for actn in action:
            if max_val <= self.getQValue(state, actn) or max_val == float("-inf"):
                max_val = self.getQValue(state, actn)
    ...
    def computeActionFromQValues(self, state):
        ...
        action_at_state = ""
        for actn in action:
            if max_value <= self.getQValue(state, actn) or max_value == float("-inf"):
                max_value = self.getQValue(state, actn)
                action_at_state = actn
    ...
    def getAction(self, state):
        ...
        """
        # Pick Action
        lgl_actns = self.getLegalActions(state)
        actn = None
        "*** YOUR CODE HERE ***"
        if len(lgl_actns) == 0:
            return None
        if util.flipCoin(self.epsilon):
            actn = random.choice(lgl_actns)
        else:
            actn = self.computeActionFromQValues(state)
        return actn
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        qval_prev = (1-self.alpha) * self.getQValue(state, action)
        qval_new = self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
        self.qvalues[(state, action)] = qval_prev + qval_new
